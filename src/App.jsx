...
/**
 * Hotel Daily Control - Responsive MVP
 *
 * Objetivo:
 * - App interna para recepción, jefa de recepción y dirección.
 * - Responsive para móvil, tablet y escritorio.
 * - Sin dependencias externas de iconos para evitar fallos CDN.
 * - Persistencia demo en localStorage.
 * - Preparada conceptualmente para Supabase + Netlify.
 */

const STORAGE_KEY = "hotel_daily_control_responsive_v1";
...
function runSelfTests() {
  const tests = [];
  const occ = calculateOccupancy({ total: 10, occupied: 8, blocked: 1 });
  tests.push({ name: "Ocupación 8/10 = 80%", pass: occ === 80, value: occ });

  const available = calculateAvailable({ total: 10, occupied: 8, blocked: 1 });
  tests.push({ name: "Disponibles 10 - 8 - 1 = 1", pass: available === 1, value: available });

  const safeAvailable = calculateAvailable({ total: 5, occupied: 6, blocked: 2 });
  tests.push({ name: "Disponibles nunca es negativo", pass: safeAvailable === 0, value: safeAvailable });
  ...
  const report = buildReportText({ hotel: defaultHotel, latest: defaultReports[0], rooms: defaultRooms, occupancy: 74, available: 9, openIncidents: 2, recommendations: stable });
  tests.push({ name: "Informe incluye hotel y recomendaciones", pass: report.includes(defaultHotel.name) && report.includes("RECOMENDACIONES AUTOMÁTICAS"), value: "OK" });
...
export default function HotelDailyControlApp() {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ hotel, rooms, reports, incidents, tasks, channels }));
    } catch {
  ...
  const recommendations = useMemo(() => {
    return createRecommendations({
      ...
      bookingShare,
      directShare,
      blockedRooms: Number(rooms.blocked) || 0,
      pendingPayments: Number(latest?.pendingPayments) || 0,
      hotel,
    });
  }, [occupancy, bookingShare, directShare, rooms.blocked, latest, hotel]);

  const reportText = useMemo(() => {
    return buildReportText({ hotel, latest, rooms, occupancy, available, openIncidents, recommendations });
  }, [hotel, latest, rooms, occupancy, available, openIncidents, recommendations]);
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              {tabs.map(([id, label, icon]) => (
                <button
                  key={id}
                  onClick={() => goToTab(id)}
                  className={cls("flex items-center gap-2 rounded-2xl px-3 py-3 text-left text-sm font-semibold", active === id ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-700")}
                >
          ...
          <nav className="space-y-1">
            {tabs.map(([id, label, icon]) => (
              <button
                key={id}
                onClick={() => goToTab(id)}
                className={cls("flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-semibold transition", active === id ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900")}
              >
                <Icon name={icon} size={18} />
                {label}
              </button>
              ...
              <div className="grid gap-3">
                {incidents.map((i) => (
                  <Card key={i.id} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <select className="rounded-xl border border-slate-300 px-3 py-2 text-sm" value={i.status} onChange={(e) => setIncidents(incidents.map((x) => x.id === i.id ? { ...x, status: e.target.value } : x))}>
          ...
          {active === "setup" && (
            <div className="space-y-5 sm:space-y-6">
              <Card>
                ...
                <p className="mb-5 text-sm text-slate-500">Estos valores luego vendrían de Supabase.</p>
              ...
              <Card>
                <h3 className="mb-3 font-bold">Arquitectura recomendada</h3>
                <div className="grid gap-4 sm:grid-cols-3">
                  <div className="rounded-2xl bg-slate-50 p-4"><b>GitHub</b><p className="text-sm text-slate-500">Repositorio y control de versiones.</p></div>
                  <div className="rounded-2xl bg-slate-50 p-4"><b>Supabase</b><p className="text-sm text-slate-500">Base de datos, usuarios, roles e histórico.</p></div>
                  <div className="rounded-2xl bg-slate-50 p-4"><b>Netlify</b><p className="text-sm text-slate-500">Deploy automático desde GitHub.</p></div>
                </div>
              </Card>
              ...
              <Card>
                ...
                <p className="mb-4 text-sm text-slate-500">La demo guarda los cambios en este navegador. En producción se guardaría en Supabase.</p>
