...
function aggregateDaily(bookings: any[], dates: string[], totalRooms: number) {
  const days = dates.map((date) => {
    ...
    for (const b of active) {
      const price = Number(b.price || b.totalPrice || 0) || 0;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 1) Reservas del rango con datos de facturación.
    //    Se pide por fechas de entrada y salida para cubrir el periodo completo.
    const bookingsResp = await beds24Call(
      "getBookings",
      {
        includeInvoice: true,
        includeInfoItems: false,
        arrivalFrom: toBeds24Date(desde),
        arrivalTo: toBeds24Date(hasta),
        departureFrom: toBeds24Date(desde),
        departureTo: toBeds24Date(hasta),
      },
      apiKey,
      propKey,
    );
    const bookings = Array.isArray(bookingsResp) ? bookingsResp : bookingsResp?.bookings || [];
